  ...
  fluidRow(
    ...
    column(2,
           selectInput(inputId = "model_name",
                       label = "Select model",
                       choices = c("Quadratic", "Non-Linear","Polynomial"),
                       selected = "Quadratic")),
...
server <- function(input, output) {
  ...
  df <- reactive({
    ...
    else if(input$dataset == "Data set 2") 
    {
      ...
      x_test <- runif(n = 1000, min = -1, max = 1)
  ...
  output$Plot1 <- renderPlot({
    ...
    if (input$dataset == "Data set 1") {
      p <- ggplot(data = df()$toy_data, aes(x = inp, y = response1)) + 
        geom_point() +
        scale_color_manual(values = c("quadratic model" = "blue", "non-linear model" = "green", "polynomial model" = "red")) +
        labs(title = "Scatter plot x vs. y", y = "y", x = "x") +
        scale_y_continuous(limits = c(15, 40)) +
        scale_x_continuous(limits = c(20, 40))
      
      if (input$model_name == "Quadratic") {
        p <- p + geom_smooth(method = "lm", se = FALSE, aes(color = "quadratic model"), show.legend = FALSE)
  ...
  output$Plot2 <- renderPlot({
    ...
    if (input$model_name == "Quadratic") {
      complexities <- 2  # Linear model has no complexity parameter
    } else if (input$model_name == "Non-Linear") {
