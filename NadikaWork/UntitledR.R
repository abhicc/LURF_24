  ...
  observe({
    if (input$app_choice == "App 1") {
      ...
      output$app_main <- renderUI({
        tabsetPanel(
          ...
          tabPanel("Logistic Regression",
                   fluidRow(
                     column(10, plotOutput("tab33_plot1"))
                   )
          )
        )
      })
      ...
      output$tab33_plot1 <- renderPlot({
        ...
        # Predict on the grid
        logit_pred_grid <- predict(logit_model, newdata = grid, type = "response")
        grid$prediction <- ifelse(logit_pred_grid > 0.5, "1", "0")
